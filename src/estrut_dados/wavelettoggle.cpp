...
struct waveletTree {
    ...
    struct Bit {    // count how many indices are active
        ...
        inline int query(int i, int j) { return query(j) - query(i - 1); }
    } bt;

#define mapLeft(i) (mp[i])           /* original mapLeft */
#define mapRight(i) (i - mapLeft(i)) /* original mapRight */

    /** *beg points to the first element, *end points to after the last element (just like stl
     * default functions). Elements are in range [lo..hi], inclusive */
    waveletTree(int* beg, int* end, int lo, int hi) { // O(nlogA)
        ...
        if (lo != hi) {
            auto pivot =
                stable_partition(beg, end, [&](int x) { return x <= mi; }); // split the vector
    ...
    int kthSmallest(int k, int l, int r) {
        ...
        if (k <= inLeft)
            ...
            return R->kthSmallest(k - inLeft, mapRight(l - 1) + 1, mapRight(r));
    }
    ...
    int rangeCount(int x, int y, int l, int r) {
        ...
        if (mi >= y)
            return L->rangeCount(x, y, mapLeft(l - 1) + 1, mapLeft(r));
        else if (mi <= x)
            return R->rangeCount(x, y, mapRight(l - 1) + 1, mapRight(r));
        else
            return L->rangeCount(x, mi, mapLeft(l - 1) + 1, mapLeft(r)) +
