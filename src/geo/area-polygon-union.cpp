...
// #include "Point.h" botei no codigo da triangulacao
// #include "sideOf.h" ta aqui:
/**
 * Author: Ulf Lundstrom
 * Date: 2009-03-21
 * License: CC0
 * Source:
 * Description: Returns where $p$ is as seen from $s$ towards $e$. 1/0/-1 $\Leftrightarrow$ left/on
 * line/right. If the optional argument $eps$ is given 0 is returned if $p$ is within distance $eps$
 * from the line. P is supposed to be Point<T> where T is e.g. double or long long. It uses products
 * in intermediate steps so watch out for overflow if using int or long long. Usage: bool left =
