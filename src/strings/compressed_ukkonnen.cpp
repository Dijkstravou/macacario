...
int t[N][26],          // array of transitions (state, letter)
    ...
    tp,   // position in the string which corresponds to the position on the edge (between l[tv] and
          // r[tv], inclusive)
...
suff:; // we'll return here after each transition to the suffix (and will add character again)
    if (r[tv] < tp) { // check whether we're still within the boundaries of the current edge
        // if we're not, find the next edge. If it doesn't exist, create a leaf and add it to the
        // tree
        if (t[tv][c] == -1) {
            t[tv][c] = ts;
            l[ts] = la;
