...
int main() {
    ...
    le(p21);
    le(p22);
    if (checa_intersec_segmento(p11, p12, p21, p22)) {
        printf("0\n");
        return 0;
    }
    ld d1 = dist_seg(p11, p21, p22);
    ld d2 = dist_seg(p12, p21, p22);
    ld d3 = dist_seg(p21, p11, p12);
    ld d4 = dist_seg(p22, p11, p12);
    ld ans = min(min(d1, d2), min(d3, d4));
    printf("%.10Lf\n", ans);
}