// https://acm.timus.ru/problem.aspx?space=1&num=1848&locale=en
// deu AC
...
vector<Edge*> sweepline(vector<Edge*> planar, vector<pt> queries) {
    ...
    for (int x = 0; x < cid; x++) {
        ...
        for (Event event : events[x]) {
            ...
            if (event.type == VERT) {
                vert.insert(
                    make_pair(min(planar[event.pos]->l.y, planar[event.pos]->r.y), event.pos));
            }
            if (event.type == ADD) {
...
vector<pair<int, int>> point_location(DCEL& planar, vector<pt> queries) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        pos[(intptr_t)e] = lt(planar.body[i]->origin.x, planar.body[i]->twin->origin.x)
                               ? planar.body[i]->face
...
int main() {
    ...
    for (int i = 0; i < num_pols; i++) {
        ...
        for (int j = 0; j < verts_pol; j++) {
            ...
            num_face.push_back(i + 1);
            planar.body[planar.body.size() - 2]->twin = planar.body[planar.body.size() - 1];
            planar.body[planar.body.size() - 1]->twin = planar.body[planar.body.size() - 2];
        }
        for (int j = ind_first_edge; j < planar.body.size(); j++) {
            int nxt_id =
                j + 2 < planar.body.size() ? j + 2 : (ind_first_edge + j + 2 - planar.body.size());
